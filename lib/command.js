...
function processArguments(args, commandName, RedisMock) {
  ...
  commandArgs = commandArgs.map(
    ...
    function(arg) {
      return arg instanceof Buffer || arg === null || arg === undefined
        ? arg
        : arg.toString();
    }
  );
...
function safelyExecuteCommand(commandEmulator, commandName, RedisMock) {
  ...
  for (
    var _len = arguments.length,
      commandArgs = Array(_len > 3 ? _len - 3 : 0),
      _key = 3;
    _key < _len;
    _key++
  ) {
...
function command(commandEmulator, commandName, RedisMock) {
  return function() {
    ...
    return (0, _standardAsCallback2.default)(
      new Promise(function(resolve) {
        return resolve(
          safelyExecuteCommand.apply(
            undefined,
            [commandEmulator, commandName, RedisMock].concat(
              _toConsumableArray(commandArgs)
            )
          )
        );
      }),
